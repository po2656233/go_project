...
func productGame(kindID, level uint32) interface{} {
	switch kindID {
	case Baccarat:
		return baccarat.NewBaccarat(level)
	case FishLord:
	case Landlords:
		return landlord.NewLandlord(level)
	case CowCow:
		return cowcow.NewCowcow(level)
	case Mahjong:
		return mahjong.NewMahjong(level)
	case ChinessChess:
		return chineseChess.NewChineseChess(level)
	}
